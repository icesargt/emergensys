...
<div class="container-fluid">
		<div class="col-lg-12">
			<div class="col-lg-6">
				<h3>Enviar Mensaje de Emergencia</h3>	
			</div>
	...
	<div class="col-lg-12">
		<div class="center-block" style="width:50%;">
			...
			<form data-parsley-validate class="form-horizontal" action="{{ route('sendnotify') }}" method="POST" autocomplete="off" novalidate="">
				...
				<div class="form-group {{ $errors->has('telefono') ? 'has-error' : '' }}">
					...
					<input type="number" id="telefono" class="form-control" name="telefono" placeholder="2440 9090" value="{{ $patient_show->mobile_number }}" required="required">
					<span class="text-danger">{{ $errors->first('telefono') }}</span>
				</div>

				<div class="form-group {{ ($errors->has('mensaje')) ? 'has-error' : '' }}">                    
                    <label for="lblmensaje">Mensaje *:</label>
                        <textarea required  class="form-control" name="mensaje" id="mensaje" cols="30" rows="3">{{ 'Esta es una emergencia. '. $patient_show->first_name .' '. $patient_show->last_name .'. '.  'Ha sido Internado en Centro Medico Zona 10. Llamar al 44205704.' }}</textarea>
