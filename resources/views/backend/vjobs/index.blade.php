@extends('admin.layout')
	@section('content')
		<div class="container-fluid">
			<div class="col-lg-12">
				<h3>Lista de puestos de trabajo</h3>
			
				<div class="col-lg-7">
					@include('backend.vjobs.search_job')
				</div>
				<div class="col-lg-5">
					<a href="{{ route('positions.create') }}" class="btn btn-success btn-md pull-right">
			...
			<div class="table-responsive">				
				<div class="col-lg-4">
					@if($list_jobs->total() > 0)
						<h4>Hay <span class="badge label label primary">{{ $list_jobs->total() }}</span> registros.</h4>
					@else
						...
						<tbody>
							@foreach($list_jobs as $job)
								<tr>
									...
									<td>
										<form class="" action="{{route('positions.destroy', $job->id_job )}}" method="post">
			                                <input type="hidden" name="_method" value="delete">
			                                <input type="hidden" name="_token" value="{{ csrf_token() }}">
			                                <a href="{{route('positions.edit', $job->id_job )}}" class="btn btn-primary btn-sm"><span class="glyphicon glyphicon glyphicon-edit" aria-hidden="true"></span> Editar</a>
					...
					{{ $list_jobs->appends( Request::only('descripcion'))->render() }}
				</div>
			</div>
		</div><!-- /container-fluid -->
@stop